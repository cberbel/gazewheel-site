...
/* Contas para uma verba de US$ 5 por mês, no Haiku 4.5
   (US$ 1 por milhão de tokens de entrada, US$ 5 por milhão de saída):
   cada pedido gasta ~1.400 tokens de entrada e ~90 de saída, ou seja
   ~US$ 0,002 no pior caso (sem cache). 80 por dia dá ~US$ 4,80 no mês.
   Se um dia estourar, o teclado não quebra: volta a ser interpretado
   dentro do navegador, como era antes. */
const DAILY_LIMIT = 15;          /* por pessoa, por dia */
const GLOBAL_DAILY = 80;         /* do site inteiro, por dia — protege a verba */
...
async function overLimit(env, req){
  if(!env.RATE) return "sem-kv";
  const ip = req.headers.get("cf-connecting-ip") || "anon";
  const day = new Date().toISOString().slice(0, 10);

  const todos = parseInt(await env.RATE.get(day + ":total") || "0", 10);
  if(todos >= GLOBAL_DAILY) return "verba";

  const key = `${day}:${ip}`;
  const n = parseInt(await env.RATE.get(key) || "0", 10);
  if(n >= DAILY_LIMIT) return "pessoa";

  await env.RATE.put(key, String(n + 1), {expirationTtl: 172800});
  await env.RATE.put(day + ":total", String(todos + 1), {expirationTtl: 172800});
  return false;
}
...
export default {
  async fetch(req, env){
    ...
    const limite = await overLimit(env, req);
    /* Estourou a verba do dia: responde como servidor fora do ar. O site
       cai sozinho para o interpretador local e a pessoa continua ajustando
       o teclado — só sem a ajuda do modelo. */
    if(limite === "verba") return json({reply: ""}, 503, origin);
    if(limite === "pessoa"){
